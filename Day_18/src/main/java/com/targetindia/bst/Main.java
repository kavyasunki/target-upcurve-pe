    ...
    public static void main(String[] args) {
        ...
        while ((choice = menu()) != 0) {
            ...
            switch (choice) {
                ...
                case 7:
                    int smallestValue = bst.findSmallest();
                    System.out.println("Smallest value in the tree: " + smallestValue);
                    break;
                case 8:
                    int largestValue = bst.findLargest();
                    System.out.println("Largest value in the tree: " + largestValue);
                    break;
                case 9:
                    int valueToFindDepth = KeyboardUtil.getInt("Enter value to find its depth: ");
                    int depth = bst.findDepth(valueToFindDepth);
                    if (depth == -1) {
                        System.out.println("Value not found in the tree.");
                    } else {
                        System.out.println("Depth of the value " + valueToFindDepth + " is: " + depth);
                    }
                    break;
                case 10:
                    int length = bst.treeLength();
