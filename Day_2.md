...
SELECT 
    LPAD(MAX(SALARY), 10, '0') AS Maximum_Salary, 
    LPAD(MIN(SALARY), 10, '0') AS Minimum_Salary 
FROM 
    employees;
...
SELECT 
    SUBSTRING(FIRST_NAME, 1, 8) AS First_Name, 
    CONCAT('$', FLOOR(SALARY / 1000), 'K') AS Salary 
FROM 
    employees 
ORDER BY 
    SALARY DESC;
